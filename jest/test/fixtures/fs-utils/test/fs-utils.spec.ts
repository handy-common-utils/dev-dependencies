...
describe('fs-utils', () => {
  it('should replaceWithFileContent(...) keep file unchanged when there is no match', async () => {
    ...
    expect(fs.readFileSync(originalFile.name).toString()).toEqual(originalFileContent);
  });
  it('should replaceWithFileContent(...) does the job when there are matches', async () => {
    ...
    expect(fs.readFileSync(originalFile.name).toString()).toEqual(originalFileContent.replace('<replace me>', contentFileContent).replace('<replace me>', contentFileContent));
  });
  it('should addSurrounding(...) does the job when there are matches', async () => {
    ...
    expect(fs.readFileSync(originalFile.name).toString()).toEqual(originalFileContent.replace(/<start>/g, '<start><body>').replace(/<end>/g, '</body><end>'));
  });
  ...
  it('should replaceInFile(...) does the job when there are matches', async () => {
    ...
    expect(fs.readFileSync(originalFile.name).toString()).toEqual(originalFileContent.replace(/<replace me>/g, '[replaced]'));
  });
  ...
  it('should replaceInFiles(...) succeed when there are two files specified', async () => {
    ...
    expect(fs.readFileSync(originalFile.name).toString()).toEqual(originalFileContent.replace(/ is /g, '[replaced]'));
    expect(fs.readFileSync(contentFile.name).toString()).toEqual(contentFileContent.replace(/ is /g, '[replaced]'));
  });
});
